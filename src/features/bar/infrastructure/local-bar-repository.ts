...
import { isBarDatabase, isOneOf, isRecord } from './bar-database-schema'
import { createDemoDatabase, createEmptyDatabase } from './demo-seed'
...
export class LocalBarRepository implements BarRepository {
  ...
  /**
   * Grava o banco vazio — grava, não apaga. A diferença é o ponto todo:
   * `load()` repõe a demonstração quando não encontra bytes, então remover
   * a linha traria Ana Paula de volta. Só um documento vazio salvo faz o
   * bar ficar vazio.
   */
